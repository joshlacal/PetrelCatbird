...
extension ATProtoClient.Blue.Catbird.Chat {
    ...
    public func getLeafRecoveryInbox(input: BlueCatbirdChatGetLeafRecoveryInbox.Parameters) async throws -> (responseCode: Int, data: BlueCatbirdChatGetLeafRecoveryInbox.Output?) {
        ...
        let serviceDID = await networkService.getServiceDID(for: "blue.catbird.chat.getLeafRecoveryInbox")
        ...
        if (200...299).contains(responseCode) {
            ...
            do {
                ...
                let decodedData = try decoder.decode(BlueCatbirdChatGetLeafRecoveryInbox.Output.self, from: responseData)

                return (responseCode, decodedData)
            } catch {
                // Log the decoding error for debugging but still return the response code
                LogManager.logError("Failed to decode successful response for blue.catbird.chat.getLeafRecoveryInbox: \(error)")
            ...
            if let atprotoError = ATProtoErrorParser.parse(
                ...
                errorType: BlueCatbirdChatGetLeafRecoveryInbox.Error.self
            ) {
                throw atprotoError
            }
